...
class MazeGame:
    ...
    def __init__(self, gsx: int, gsy: int, m: Mach2):
        ...
        self.grid = []
        for i in range(0, gsy):
            self.grid.append([0] * gsx)
    ...
    def move(self, direction: int):
        ...
        if out == 0:
            self.draw_rectangle(npos, "white", 0.25)
        if out == 1:
            self.set_turtle_pos(npos)
            self.droid_pos = npos
        self.operating = False
    ...
    def play_maze_auto(self, iteration: int, dir: int):
        res = self.move(dir)
        if res == 0:
            return
        if res == 2:
            print("found solution len ", iteration, "at pos", self.oxy_pos.x, self.oxy_pos.y)
            self.solutions.append(iteration)

        for i in range(0, 4):
            if i == self.backtrack[dir]:
                continue
            self.play_maze_auto(iteration + 1, i)
...
def main(lines: List[str]):
    ...
    for i in range(0, 4):
        maze.play_maze_auto(1, i)

    print("smallest solution len:", min(maze.solutions))
    turtle.update()
