...
def path_machine(line: str) -> List[Point]:
    ...
    for elem in codes:
        if elem.ev == "R":
            for x in range(x + 1, x + elem.num + 1):
                path.append(Point(x, y))
        elif elem.ev == "L":
            for x in range(x - 1, x - elem.num -1, -1):
                path.append(Point(x, y))
        elif elem.ev == "U":
            for y in range(y - 1, y - elem.num - 1, -1):
                path.append(Point(x, y))
        elif elem.ev == "D":
            for y in range(y + 1, y + elem.num + 1):
                path.append(Point(x, y))
    return path
...
def main(lines: List[str]):
    ...
    for elem in intersec:
        print(l1.index(elem), l2.index(elem))
        dist = l1.index(elem) + l2.index(elem)
        if dist != 0:
